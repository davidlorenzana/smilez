...
c1ccccc1O""".splitlines()


class TestDictionaries(unittest.TestCase):
    def test_dictionary_ids(self):
        self.assertEqual(smilez.BYTE_DICTIONARY, 0)
        self.assertEqual(smilez.WHITESPACE_DICTIONARY, 1)

    def test_num_dictionaries(self):
        self.assertEqual(smilez.NUM_DICTIONARIES, 2)
    
    def test_default_dictionary(self):
        s = smilez.compress("P")
        self.assertEqual(s, " ")
        s = smilez.compress("PP")
        self.assertEqual(s, "  ")

    def test_byte_dictionary(self):
        s = smilez.compress("P", 0)
        self.assertEqual(s, " ")
        s = smilez.compress("PP", 0)
        self.assertEqual(s, "  ")

    def test_whitespace_dictionary(self):
        s = smilez.compress("P", 1)
        self.assertEqual(s, "\xfeP")  # Single character escape
        s = smilez.compress("PP", 1)
        self.assertEqual(s, "\xff\x01PP")  # Verbatim escape

    def test_dictionary_out_of_range(self):
        for i in (-10, -1, 2, 3, 10):
            try:
                smilez.compress("C", i)
                raise AssertionError(i)
            except ValueError, err:
                self.assertEqual(str(err), "dictionary value is out of range")


class TestCompression(unittest.TestCase):
    def test_byte_round_trip(self):
        uncompressed_size = sum(map(len, smiles_list))
        sizes = {}
        for dictionary in (0, 1):
            size = 0
            for smiles in smiles_list:
                compressed_smiles = smilez.compress(smiles, dictionary)
                size += len(compressed_smiles)
                new_smiles = smilez.decompress(compressed_smiles)
                self.assertEqual(smiles, new_smiles, (smiles, new_smiles, dictionary))
            sizes[dictionary] = size

        dict0_ratio = (sizes[0]+0.0) / uncompressed_size
        dict1_ratio = (sizes[1]+0.0) / uncompressed_size

        self.assertTrue(0.40065 < dict0_ratio < 0.40066, dict0_ratio)
        self.assertTrue(0.40160 < dict1_ratio < 0.40170, dict1_ratio)
